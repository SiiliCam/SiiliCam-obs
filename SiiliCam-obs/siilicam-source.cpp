...
static bool switch_camera_button_clicked(obs_properties_t *props,
  ...
  return true; // Return true to indicate that properties should be refreshed
               // (optional).
}

static bool aspect_ratio_button_clicked(obs_properties_t *props,
                                        obs_property_t *property, void *data,
  ...
  obs_source_t *source =
      obs_get_source_by_name(custom->obs_source_name.c_str());
  if (!source) {
...
static void custom_get_defaults(obs_data_t *settings) {
  ...
  obs_data_set_default_int(settings, "aspect_ratio_height",
                           50); // Default height
}
...
static obs_properties_t *custom_get_properties(void *data) {
  ...
  obs_property_t *p =
      obs_properties_add_list(props, "ndi_source_list", "NDI Sources",
  ...
  obs_property_t *switch_camera_button = obs_properties_add_button(
      props, "switch_camera", "Switch Camera", switch_camera_button_clicked);
  add_aspect_ratio_properties(props, data);
  return props;
}
gs_texture_t *create_solid_color_texture(uint32_t width, uint32_t height,
                                         uint32_t color) {
  gs_texture_t *texture =
      gs_texture_create(width, height, GS_RGBA, 1, nullptr, GS_DYNAMIC);
  if (!texture) {
    return nullptr;
  }
  ...
  gs_texture_set_image(texture, reinterpret_cast<uint8_t *>(data), width * 4,
                       false);
  delete[] data;
...
static void custom_video_render(void *data, gs_effect_t *effect) {
  ...
  gs_texture_t *ndi_texture = gs_texture_create(ndiFrame.width, ndiFrame.height,
                                                GS_RGBA, 1, &dataPtr, 0);
  if (!ndi_texture) {
...
static void custom_update(void *data, obs_data_t *settings) {
  ...
  std::string selectedNdiSource =
      obs_data_get_string(settings, "ndi_source_list");
  std::lock_guard<std::mutex> lock(customDataMutex);
  if (selectedNdiSource != custom->selected_ndi_source) {
    custom->selected_ndi_source =
        obs_data_get_string(settings, "ndi_source_list");
...
static void *custom_create(obs_data_t *settings, obs_source_t *source) {
  ...
  // this magic command will make the pauses in timestamps not make delay in the
  // source output
  ...
  data->ndiReceiver->addAudioCallback([data](common_types::Audio audio) {
    ...
    obs_audio_frame.data[0] =
        reinterpret_cast<const uint8_t *>(audio.data.data());

    obs_audio_frame.frames = audio.noSamples;
    obs_audio_frame.speakers =
        SPEAKERS_STEREO; // Adjust based on actual channel count
  ...
  data->ndiReceiver->addFrameCallback([data](common_types::Image image) {
    ...
    // Assuming the data is tightly packed, modify if stride (bytes per line) is
    // different
  ...
  data->ndiReceiver->setVideoConnectedCallback(
      [data](Image img) { obs_source_set_enabled(data->source, true); });
  data->ndiReceiver->setVideoDisconnectedCallback(
      [data]() { obs_source_set_enabled(data->source, false); });
  return data;
}
...
static void custom_destroy(void *data) {
  ...
  std::lock_guard<std::mutex> lock(customDataMutex);
  sharedCustomDataVector.erase(std::remove(sharedCustomDataVector.begin(),
                                           sharedCustomDataVector.end(), data),
                               sharedCustomDataVector.end());
}
